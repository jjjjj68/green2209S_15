...
<script>
  ...
  function pageCheck() {
  	...
  	location.href = "${ctp}/gesipan/gesipan?pageSize="+pageSize+"&pag=${pageVo.pag}";
  }
...
<div class="container text-center">
	<table>
		<tr>
			<td class="text-left p-0" style="float: left;">
     		<a href="${ctp}/gesipan/input?pag=${pageVo.pag}&pageSize=${pageVo.pageSize}" style="margin-top: 50px;" class="btn btn-secondary btn-sm">글쓰기</a>
   		</td>		
		</tr>
	</table>
  <form name="searchForm" method="post" action="${ctp}/boSearch.bo"  style="margin-bottom: 30px; float: right;">
    ...
    <input type="text" name="searchString" id="searchString" title="검색어 입력" style="width: 300px;" placeholder="검색어 키워드를 입력해주세요"/>
...
<form name="myform">
	<div>
		<table class="text-dark table center" style="width: 60%; margin: auto; padding-right: 900px; ">
	    <tr class="top">
	      <th>글번호</th>
	      <th>글제목</th>
	      <th>글쓴이</th>
	    ...
	    <c:forEach var="vo" items="${vos}">
		   	<tr>
			   	<td>${curScrStartNo}</td>
			   	<td class="text-left">
	    	    <a href="${ctp}/gesipan/content?idx=${vo.idx}&pageSize=${pageVo.pageSize}&pag=${pageVo.pag}">${vo.title}</a>
    	  	</td>
		      <td>${vo.mid}</td>
